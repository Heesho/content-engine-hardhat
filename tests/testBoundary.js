  ...
  describe("Rewarder Balance Boundary Tests", function () {
    it("Should handle deposit when totalSupply is 0", async function () {
      ...
      if (price.gt(0)) {
        ...
        const balance = await rewarder.account_Balance(user2.address);
    ...
    it("Should handle withdrawal that brings balance to 0", async function () {
      ...
      if (price.gt(0)) {
        ...
        const user3BalanceAfter = await rewarder.account_Balance(user3.address);
        ...
        const user3BalanceFinal = await rewarder.account_Balance(user3.address);
